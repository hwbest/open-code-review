...
### Слияние нескольких файлов правил {#merging-multiple-rule-files}

Поле `rule` принимает либо строку (инлайн-текст или путь к одному файлу `.md`),
либо **массив строк**, объединяющий несколько документов правил в один текст —
например, общие + языковые + доменные правила, уложенные вместе. Правило
первого совпадения не меняется: по-прежнему применяется одна победившая
запись — просто теперь она может нести несколько файлов.
...
Элементы массива могут смешивать пути к файлам и инлайн-текст:
...
- **Одиночное значение** (строка или массив из одного элемента) — выдаётся как
  есть, без разделителя, побайтово идентично прежнему поведению.
- **Несколько файлов** — сегменты соединяются пустой строкой + `---` + пустой
  строкой. Слитый текст правила содержит только содержимое, без заголовков для
  каждого файла — так случаи одиночного значения, массива из одного элемента и
  массива из нескольких элементов единообразны. Отслеживание источника файла
  несёт строка `Rule Files:`, которую печатает `ocr rules check` (она никогда
  не попадает в промпт LLM), а не текст правила.
- Отсутствующий или пустой элемент выбрасывается из слитого **текста правила**
  (с предупреждением); если выпадают все элементы, запись откатывается к
  системному правилу (та же семантика, что у одного отсутствующего файла).
  Выброшенный **путь файла**
  по-прежнему перечисляется в строке `Rule Files:` как вы его написали, так что
  опечатанный путь, ничего не внёсший, остаётся видимым. Пустой/состоящий только
  из пробелов элемент массива (например, явная "" или "  ") тоже выбрасывается, но
  из строки `Rule Files:` удаляется полностью — он не является ни файлом, ни
  инлайн-текстом.

> Пути файлов внутри массива по-прежнему подчиняются ограничению обхода путей
> из #1100: ссылки уровня проекта не могут выйти за корень репозитория.

Два предела ограничивают недоверенный массив, чтобы он не раздувал I/O или память:

- **Количество** — массив длиннее 32 элементов усекается до первых 32 до
  чтения любого файла; остальные отбрасываются с предупреждением. Пустые
  элементы идут в зачёт этих 32, так что заполнение пустышками лишь тратит бюджет.
- **Размер** — слитый текст правила ограничен 512 КБ (524288 байт). Если вход
  переводит итог за предел, этот вход пропускается (более поздние, меньшие
  входы ещё помещаются) с предупреждением; один файл крупнее 512 КБ
  отклоняется целиком с предупреждением (пропускается из слияния, как
  отсутствующий файл). Если после этого не остаётся ни одного внёсшего
  вклад входа, запись откатывается к системному правилу.
...
Строка `Rule Files:` перечисляет, из каких файлов пришло совпавшее правило
(инлайн-текст показывается как `<inline>`), делая слитое содержимое
отслеживаемым. Используйте это всякий раз, когда правило ведёт себя не так, как
ожидалось — команда показывает **уровень** и **шаблон**, который победил.
...
### Уровень проекта: укладка языковых правил и правил по аспектам

Уложите несколько документов правил на один путь вместо того, чтобы впихивать
всё в один файл. Для каждого `path` сочетайте языковое базовое правило с любой
комбинацией правил по аспектам — security, performance, convention:
...
`rules/java.md`, `rules/cpp.md`, `rules/kotlin.md` несут специфичное для языка
базовое правило, хранимое для каждого `path`; `rules/security.md`,
`rules/performance.md`, `rules/convention.md` — переиспользуемые правила по
аспектам, которые вы комбинируете по необходимости. `ocr rules check` печатает
строку `Rule Files:`, чтобы вы могли подтвердить, какая комбинация реально
применилась.
